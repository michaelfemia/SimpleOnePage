...
$(window).scroll(function() {
    ...
    if (windowWidth > 450 && ($("#pageprogress").length > 0)) {
        var docBegin = windowHeight;
        var docEnd = $("#htmlWrapper").height() - ($("#contentWrapper").height() * .15);
        var progress = 100 * (scrollHeight / docEnd) + "%";
        if (scrollHeight > docBegin || scrollHeight < docEnd) {
            if (docEnd > 1500) {
        ...
        if ((scrollHeight < docBegin || scrollHeight > docEnd) && ($(".subheader").length > 2)) {
            $("#pagechapters").hide();
        }
    }
});
